...
final class EditorReportesModel
{
    ...
    public function fuentes(): array
    {
        return [
            'personal' => [
                'titulo' => 'Funcionarios',
                'descripcion' => 'Datos generales del personal con columnas alineadas al EDITOR DE REP del PBL.',
                'default' => ['cedula', 'funcionario', 'rango', 'dependencia', 'sexo', 'estado', 'fecha_ingreso', 'tiempo_servicio'],
                'columnas' => [
                    'nemp' => ['titulo' => 'N. empleado', 'sql' => "COALESCE(CAST(e.legacy_position AS CHAR), e.external_agent_number, CAST(e.id AS CHAR))"],
                    'cedula' => ['titulo' => 'CEDULA', 'sql' => 'e.document_number'],
                    ...
                    'tipo_policia' => ['titulo' => 'T. POLICIA', 'sql' => 'e.external_user_type'],
                    'fecha_ingreso' => ['titulo' => 'F. INGRESO', 'sql' => 'e.hire_date'],
                    'fecha_ascenso' => ['titulo' => 'F. ASCENSO', 'sql' => 'e.promotion_date'],
                    'fecha_estado' => ['titulo' => 'F. TRASLADO', 'sql' => 'e.status_date'],
                    'fecha_vacaciones' => ['titulo' => 'F. ULTIMAS VACACIONES', 'sql' => 'e.vacation_date'],
                    'fecha_nacimiento' => ['titulo' => 'F. NACIMIENTO', 'sql' => 'e.birth_date'],
                    'edad' => ['titulo' => 'EDAD', 'sql' => "CASE WHEN e.birth_date IS NULL THEN '' ELSE CAST(TIMESTAMPDIFF(YEAR, e.birth_date, CURDATE()) AS CHAR) END"],
                    'jubilacion_futura' => ['titulo' => 'F. JUBILACION FUTURA', 'sql' => "CASE WHEN e.hire_date IS NULL THEN '' ELSE CAST(DATE_ADD(e.hire_date, INTERVAL 30 YEAR) AS CHAR) END"],
                    'tiempo_servicio' => ['titulo' => 'T. SERVICIO', 'sql' => "CASE WHEN e.hire_date IS NULL THEN '' ELSE CONCAT(TIMESTAMPDIFF(YEAR, e.hire_date, CURDATE()), ' años') END"],
                    'tiempo_rango' => ['titulo' => 'T. RANGO', 'sql' => "CASE WHEN e.promotion_date IS NULL THEN '' ELSE CONCAT(TIMESTAMPDIFF(YEAR, e.promotion_date, CURDATE()), ' años') END"],
                    'tipo_dependencia' => ['titulo' => 'T. DEPENDENCIA', 'sql' => "COALESCE(u.name, e.legacy_unit_name, e.external_substation_name, '')"],
                    'cargo' => ['titulo' => 'CARGO', 'sql' => "''"],
                    'funcion' => ['titulo' => 'FUNCION', 'sql' => "''"],
                    'academia' => ['titulo' => 'ACADEMIA', 'sql' => "''"],
                    'tipo_oficial' => ['titulo' => 'T. OFICIAL', 'sql' => "''"],
                    'estado_especial' => ['titulo' => 'EST. ESPECIAL', 'sql' => "''"],
                    'tipo_estatus_especial' => ['titulo' => 'T. ESTATUS.ESP', 'sql' => "''"],
                ],
            ],
            'acciones' => [
